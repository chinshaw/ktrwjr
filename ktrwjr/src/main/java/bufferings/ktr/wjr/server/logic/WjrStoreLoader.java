...
public abstract class WjrStoreLoader {
	...
	public WjrStore loadWjrStore(String searchRootDirPath) {
		checkNotNull(searchRootDirPath,
				"The searchRootDirPath parameter is null.");

		// Commenting this block because it is not needed anymore due to using
		// guava reflection utils.
		// File classesDir = new File(searchRootDirPath);
		// checkArgument(classesDir.exists(), "%s not exists.",
		// searchRootDirPath);
		// checkArgument(classesDir.isDirectory(), "%s isn't a directory.",
		// searchRootDirPath);

		WjrStore wjrStore = new WjrStore();
		findAndStoreTests(wjrStore, "bufferings.ktr");
	 ...
	 */
	protected void findAndStoreTests(WjrStore store, String searchPackage) {
		ClassPath classpath = null;
		try {
			classpath = ClassPath.from(WjrStoreLoader.class.getClassLoader());
		} catch (IOException e) {
			throw new RuntimeException(
					"Unable to create classpath loader for finding modules", e);
		}
	...
	protected String getClassNameFromClassFile(String searchRootDirPath,
		...
		return path.substring(beginIndex, endIndex).replaceAll(
				FILE_SEPARATOR_REGEX, PACKAGE_SEPARATOR);
	}
	 ...
	 *            The class.
	 */
	protected abstract void checkAndStoreTestClass(WjrStore store,
			Class<?> clazz);
}
